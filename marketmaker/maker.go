...
func (mm *MarketMaker) seedMarket() error {
	currentPrice := simulateFetchCurrentINNPrice()

	logrus.WithFields(logrus.Fields{
		"currentINNPrice": currentPrice,
...
// the current INN price so we can offset both for bid and ask.
func simulateFetchCurrentINNPrice() float64 {
